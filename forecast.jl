...
@inline function find_length(ts::AbstractArray{T,2}) where {T<:AbstractFloat}
	return any(isnan, ts) ? (findfirst(isnan, ts[1,:]) - 1) : size(ts,2)
end	
...
function attempt_forecast(i, target, data, overlap, threshold)
	flag = false
...
function forecast(type::Type{NaiveSingle}, X::AbstractArray{T,3}, target_length, window, offset) where {T<:AbstractFloat}
...
function forecast(type::Type{NaiveMultiple}, X::AbstractArray{T,3}, target_length, max_overlap, min_overlap, threshold) where {T<:AbstractFloat}
	n_instances, n_columns, _ = size(X)
